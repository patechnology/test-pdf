          ...
          if (submission) {
            ...
            if (code_quality) {
              ...
              if (codesmells) {
                ...
                codeSmellIssues += codesmells.total;
              }
              //End => Codesmells
            }
...
function fillCandidatesTableTopSection(candidates) {
  for (var i = 0; i < candidates.length; i++) {
    ...
    $('#candidates_Questions').append('<tr data-candidateindex="' + i + '">' +
      ...
      '<div class="progress-bar" role="progressbar" style="width:' + (candidate.last_score*100)/candidate.total_score + '%; height:10px; background:#00bcd4"></div>' +
      '</div>' + (candidate.last_score) + '/' + candidate.total_score+ '</td>' +
...
function appendCodeSmellContent(codesmells, tableCSContent) {
  ...
  codesmells.issues.forEach(function (ind3, v) {
    ...
    if (!issueMap[ind3.rule] && (ind3.severity === 'MAJOR' || ind3.severity === 'MINOR')) {
      tableCSContent += '<tr><td>' + csType + '</td><td><div>' + csMessage + '</div></td><td><div style="max-width:450px;">' + csComponent + '</div></td></tr>';
      issueMap[ind3.rule] = ind3;
    }
  });
  var issues = []
  for (let idx = 0; idx < codesmells.issues.length; idx++) {
    const issue = codesmells.issues[idx];
    if (issueMap[issue.rule]) {
      issues = issueMap[issue.rule];
    }
  }
